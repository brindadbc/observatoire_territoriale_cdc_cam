...
//         return view('observatoire.departement', compact(
//             'departement', 'stats', 'communes', 'evolutionPerformances', 'annee'
//         ));
//     }
    
//     private function getTauxMoyenRealisationDept($departementId, $annee)
//     {
//         return Taux_realisation::whereHas('commune', function($q) use ($departementId) {
...
//     {
//         $dettesCnps = dette_cnps::whereHas('commune', function($q) use ($departementId) {
//             $q->where('departement_id', $departementId);
//         })->whereYear('date_evaluation', $annee)->sum('montant');
        
//         $dettesFiscale = dette_fiscale::whereHas('commune', function($q) use ($departementId) {
//             $q->where('departement_id', $departementId);
//         })->whereYear('date_evaluation', $annee)->sum('montant');
        
//         $dettesFeicom = dette_feicom::whereHas('commune', function($q) use ($departementId) {
//             $q->where('departement_id', $departementId);
//         })->whereYear('date_evaluation', $annee)->sum('montant');
        
//         $dettesSalariale = dette_salariale::whereHas('commune', function($q) use ($departementId) {
//             $q->where('departement_id', $departementId);
//         })->whereYear('date_evaluation', $annee)->sum('montant');
...
//             ->whereHas('tauxRealisations', function($q) use ($annee) {
//                 $q->where('annee_exercice', $annee)->where('pourcentage', '>=', 75);
//             })->count();
...
//             ->with([
//                 'receveurs', 'ordonnateurs', 'depotsComptes', 
//                 'previsions', 'realisations', 'tauxRealisations',
...
//                 $realisationTotal = $commune->realisations->where('annee_exercice', $annee)->sum('montant');
//                 $tauxRealisation = $commune->tauxRealisations->where('annee_exercice', $annee)->first();
                
//                 // Calcul des dettes totales
//                 $dettesTotal = $commune->dettesCnps->whereYear('date_evaluation', $annee)->sum('montant') +
//                               $commune->dettesFiscale->whereYear('date_evaluation', $annee)->sum('montant') +
//                               $commune->dettesFeicom->whereYear('date_evaluation', $annee)->sum('montant') +
//                               $commune->dettesSalariale->whereYear('date_evaluation', $annee)->sum('montant');
...
//                     'status' => $this->determinerStatusCommune($commune, $annee)
//                 ];
//             });
//     }
...
//             ->selectRaw('annee_exercice, AVG(pourcentage) as taux_moyen')
//             ->groupBy('annee_exercice')
//             ->orderBy('annee_exercice')
//             ->get();
//     }
    
//     private function determinerStatusCommune($commune, $annee)
//     {
...
//         if ($defaillances > 0 || $retards > 0) return 'Non conforme';
//         if (!$tauxRealisation || $tauxRealisation->pourcentage < 50) return 'Non conforme';
