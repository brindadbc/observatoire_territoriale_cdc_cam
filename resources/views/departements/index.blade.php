...
<div class="departements-index">
    ...
    <div class="page-actions">
        <div class="filter-year">
            ...
            <select id="annee-select" class="filter-select" onchange="changeAnnee(this.value)">
    ...
    <div class="stats-grid" id="stats-container">
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h4>Total Départements</h4>
                <div class="stat-number">{{ $statsGenerales['total_departements'] }}</div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h4>Total Communes</h4>
                <div class="stat-number">{{ $statsGenerales['total_communes'] }}</div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h4>Budget Total {{ $annee }}</h4>
                <div class="stat-number">{{ number_format($statsGenerales['budget_total_national'] / 1000000, 0) }}M</div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h4>Taux Moyen {{ $annee }}</h4>
                <div class="stat-number">{{ number_format($statsGenerales['taux_moyen_national'], 1) }}%</div>
                <div class="stat-subtitle">Réalisation</div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h4>Communes Conformes {{ $annee }}</h4>
                <div class="stat-number">{{ number_format($statsGenerales['communes_conformes_national'], 1) }}%</div>
            </div>
        </div>
    </div>
    ...
    <div class="table-container">
        <table class="departements-table" id="departementsTable">
            ...
            <tbody>
                ...
                <tr class="departement-row" 
                    ...
                    <td class="taux-cell">
                        <span class="taux-badge {{ $departement->stats['taux_moyen_realisation'] >= 75 ? 'good' : ($departement->stats['taux_moyen_realisation'] >= 50 ? 'medium' : 'bad') }}">
                            {{ number_format($departement->stats['taux_moyen_realisation'], 1) }}%
                        </span>
                    </td>
                    <td class="budget-cell">
                        {{ number_format($departement->stats['budget_total'] / 1000000, 1) }}M
                    </td>
                    <td class="dettes-cell">
                        <span class="dettes-amount {{ $departement->stats['total_dettes'] > 0 ? 'has-debts' : 'no-debts' }}">
                            {{ number_format($departement->stats['total_dettes'] / 1000000, 1) }}M
                        </span>
                    </td>
                    <td class="conformite-cell">
                        <div class="conformity-indicator">
                            <span class="conformity-percent {{ $departement->stats['communes_conformes'] >= 75 ? 'good' : ($departement->stats['communes_conformes'] >= 50 ? 'medium' : 'bad') }}">
                                {{ number_format($departement->stats['communes_conformes'], 1) }}%
                            </span>
                            <div class="conformity-bar">
                                <div class="conformity-fill {{ $departement->stats['communes_conformes'] >= 75 ? 'good' : ($departement->stats['communes_conformes'] >= 50 ? 'medium' : 'bad') }}" 
                                     style="width: {{ $departement->stats['communes_conformes'] }}%"></div>
                            </div>
                        </div>
                    </td>
                    <td class="actions-cell">
                        <div class="action-buttons">
                            <a href="{{ route('departements.show', [$departement, 'annee' => $annee]) }}" class="btn btn-sm btn-outline" title="Voir détails">
                                <i class="fas fa-eye"></i>
                            </a>
                            ...
                            <button class="btn btn-sm btn-outline btn-export" 
                                    onclick="exportDepartement({{ $departement->id }}, {{ $annee }})" 
                                    title="Exporter">
        ...
        <div id="noResults" class="no-results" style="display: none;">
            <div class="no-results-content">
                <i class="fas fa-search"></i>
                <h3>Aucun département trouvé</h3>
                <p>Aucun département ne correspond à vos critères de recherche pour l'année {{ $annee }}.</p>
            </div>
        </div>
    </div>
    ...
    <!-- Pagination -->
    <div class="pagination-wrapper">
        {{ $departements->appends(['annee' => $annee])->links() }}
    </div>
</div>
...
/* Ajout des styles pour les nouvelles colonnes */
.filter-year {
    display: flex;
    align-items: center;
    gap: 0.5rem;
...
.taux-badge.good, .conformity-percent.good {
    background: rgba(16, 185, 129, 0.1);
    color: #059669;
    border: 1px solid rgba(16, 185, 129, 0.3);
}

.taux-badge.medium, .conformity-percent.medium {
    background: rgba(245, 158, 11, 0.1);
    color: #d97706;
    border: 1px solid rgba(245, 158, 11, 0.3);
}

.taux-badge.bad, .conformity-percent.bad {
...
.dettes-amount.has-debts {
    color: #dc2626;
}

.dettes-amount.no-debts {
    color: #059669;
}
...
.conformity-fill.good {
    background: linear-gradient(90deg, #10b981, #059669);
}

.conformity-fill.medium {
...
@media (max-width: 768px) {
    .taux-cell, .budget-cell, .dettes-cell, .conformite-cell {
        display: none;
    }
...
/* Animation pour le changement d'année */
.stats-grid.loading {
    opacity: 0.6;
    pointer-events: none;
}

.departements-table.loading tbody {
    opacity: 0.6;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fonction pour charger tous les départements via AJAX
    function loadAllDepartements() {
        fetch(`{{ route("departements.index") }}?all=true&annee=${currentYear}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
    ...
    function displaySearchResults(results, searchTerm) {
        ...
        const resultsHtml = results.map(dept => `
            <div class="search-result-item" data-dept-id="${dept.id}">
                ...
                <div class="search-result-info">
                    Code: ${dept.code} | Région: ${dept.region ? dept.region.nom : 'N/A'}
                    ${dept.chef_lieu ? ` | Chef-lieu: ${dept.chef_lieu}` : ''}
                    | Taux: ${dept.stats ? dept.stats.taux_moyen_realisation.toFixed(1) : '0.0'}%
                </div>
            </div>
        `).join('');
        ...
        searchResults.querySelectorAll('.search-result-item').forEach(item => {
            item.addEventListener('click', function() {
                const deptId = this.dataset.deptId;
                window.location.href = `{{ route('departements.show', ['departement' => ':id']) }}`.replace(':id', deptId) + `?annee=${currentYear}`;
            });
        });
...
function changeAnnee(annee) {
    ...
    // Rediriger avec la nouvelle année
    const currentUrl = new URL(window.location);
    currentUrl.searchParams.set('annee', annee);
    window.location.href = currentUrl.toString();
}

// Fonction d'export avec année
function exportDepartement(departementId, annee) {
    window.location.href = `/departements/${departementId}/export?annee=${annee}&format=pdf`;
}

// Actualiser les données pour l'année courante
